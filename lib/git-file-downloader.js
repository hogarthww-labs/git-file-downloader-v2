...
class GitFileDownloader {  
  setGithubRawBaseUrl(githubRawBaseUrl) {
    this.githubRawBaseUrl = githubRawBaseUrl;

    return this;
  }

  setGitlabRawBaseUrl(gitlabRawBaseUrl) {
    this.gitlabRawBaseUrl = gitlabRawBaseUrl;
   ...
   */
  constructor(options) {
    this.githubRawBaseUrl = GitFileDownloader.GithubRawBaseUrl;
    this.gitlabRawBaseUrl = GitFileDownloader.GitlabRawBaseUrl;
    ...
    switch (this.options.provider) {
      case 'github':
      default:
        this.url = this.githubRawBaseUrl + [
      ...
      case 'gitlab':
        this.url = this.gitlabRawBaseUrl + [
          this.options.repository,
          'raw',
          this.options.branch,
          this.options.file
  ...
  run() {
    return new Promise((resolve, reject) => {
      ...
      req.end((requestError, res) => {
        if (!requestError) {
          if (this.options.output === null || this.options.writeFile === false) {
            resolve(res.text);
          } else {
